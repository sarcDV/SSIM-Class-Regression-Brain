...
from utilities_new import MoCoDatasetRegression, tensorboard_regression, getSSIM
...
testdata = glob('/pool/alex/Motion-Correction-3D/samples/output/test/*.nii.gz')
...
level_noise = 0.07
testset = MoCoDatasetRegression(testdata, patches=patches, 
                       size=size_,
...
with torch.no_grad():
    ...
    for idx, (cor, img, ssimtmp) in enumerate(tqdm(test_loader)):
        ...
        pred = model(cor)
        niifiles = np.zeros((size_,2*size_, batch_size_*patches))
        for ii in range(0, ssimtmp.shape[0]):
            ...
            niifiles[:,:,ii] = np.concatenate((gt_, inp_), axis=1)
